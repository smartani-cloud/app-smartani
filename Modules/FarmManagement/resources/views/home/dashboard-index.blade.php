      ...
      <div class="card-body pt-0">
        <div class="row pt-2">
          ...
          <div class="col border-right">
            @php
              $latestValue = $s->datas()->latest('recorded_at')->first()->value;
              $matchingFeedback = $s->feedback()
                  ->where('min', '<=', $latestValue)
                  ->where('max', '>=', $latestValue)
                  ->first();
            @endphp
            <h2 class="font-weight-light">{{ $matchingFeedback ? $matchingFeedback->feedback : $latestValue }}</h2>
            <h6>{{ $latestValue.' '.$s->unit }}</h6>
            <h6><i class="fas fa-temperature-low mr-2"></i>{{ $s->name }}</h6>
          </div>
          @endforeach
        </div>
      </div>
