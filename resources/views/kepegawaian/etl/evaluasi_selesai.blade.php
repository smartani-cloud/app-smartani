                ...
                <table id="dataTable" class="table align-items-center table-flush">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      @if($e->recommend_status_id)
                      <td>
                        @if($e->recommend_status_id == 1)
                          <i class="fa fa-lg fa-check-circle text-success mr-1" data-toggle="tooltip" data-original-title="{{ ucwords($e->rekomendasiLanjut->status) }}"></i>
                        @elseif($e->recommend_status_id == 2)
                          <i class="fa fa-lg fa-times-circle text-danger" data-toggle="tooltip" data-original-title="{{ ucwords($e->rekomendasiLanjut->status) }}"></i>
                        @endif
                      </td>
                      @else
                      <td>-</td>
                      @endif
                      <td>{{ $e->rekomendasiStatus ? $e->rekomendasiStatus->acronym : '-' }}</td>
                      <td>
                        ...
                        @elseif($e->education_acc_status_id == 1)
                          <i class="fa fa-lg fa-check-circle text-success mr-1" data-toggle="tooltip" data-original-title="Disetujui oleh {{ Auth::user()->pegawai->is($e->accEdukasi) ? 'Anda' : $e->accEdukasi->name }}<br>{{ date('j M Y H.i.s', strtotime($e->education_acc_time)) }}"></i>
                        @else
                      ...
                      <td>
                        <a href="#" class="btn btn-sm btn-brand-green-dark" data-toggle="modal" data-target="#edit-form" onclick="editModal('{{ route('evaluasi.ubah') }}','{{ $e->id }}')" data-toggle="modal" data-target="#edit-form"><i class="fas fa-eye"></i></a>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-brand-green border-0">
        <h5 class="modal-title text-white">Rincian Hasil Evaluasi</h5>
