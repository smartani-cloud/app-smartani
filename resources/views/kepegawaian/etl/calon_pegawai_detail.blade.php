    ...
    <div class="card">
      ...
      <div class="card-body bg-gray-200 p-4">
        <div class="d-flex align-items-center">
          ...
          <span class="mdi mdi-24px mdi-information text-info mr-2"></span>Calon pegawai ini masih menunggu persetujuan Anda
          @else
          <span class="mdi mdi-24px mdi-check-circle text-success mr-2"></span>Rekomendasi calon pegawai ini telah disetujui oleh {{ Auth::user()->pegawai->is($calon->accEdukasi) ? 'Anda' : $calon->accEdukasi->name }}
          @endif
        </div>
      </div>
      <div class="card-body p-4">
          ...
          <div class="col-lg-10 col-md-12">
            <div class="row">
              <div class="offset-lg-3 col-lg-9 offset-md-4 col-md-8 col-12 text-left">
                <button type="button" class="btn btn-success" data-toggle="modal" data-target="#validate-confirm" onclick="validateModal('{{ addslashes(htmlspecialchars($calon->name)) }}', '{{ ucwords($calon->rekomendasiPenerimaan->status) }}', '{{ $calon->rekomendasiPenempatan ? $calon->rekomendasiPenempatan->name : '-' }}', '{{ $calon->statusPegawai ? $calon->statusPegawai->status : '-' }}', '{{ route('calon.validasi', ['id' => $calon->id]) }}')">Setujui</button>
              </div>
            </div>
          </div>
        </div>
