                ...
                <div class="row mb-2">
                  ...
                  <div class="col-lg-6 col-md-6 col-12">
                    ...
                    <select aria-label="Tahun" name="tahun" class="form-control" id="yearOpt">
                      ...
                      @if($t->is_finance_year == 1 || ($t->is_finance_year != 1 && $t->whereHas('ppa',function($q)use($jenisAktif,$t){$q->where('academic_year_id',$t->id)->whereHas('jenisAnggaranAnggaran',function($q)use($jenisAktif){$q->where('budgeting_type_id',$jenisAktif->id);})->has('lppa');})->count()))
        ...
        <div class="card">
            ...
            <div class="card-body p-3">
                ...
                <div class="row ml-1">
                    ...
                    @php
                    $ppaCount = $a->ppa()->where(['academic_year_id'=>$tahun->id,'director_acc_status_id'=>1])->has('lppa')->count();
                    $anggaranAktif += $ppaCount;
                    @endphp
                    @if($ppaCount > 0)
                    <div class="col-md-6 col-12 mb-3">
                        <div class="row py-2 rounded border border-light mr-2">
                            <div class="col-8 d-flex align-items-center">
                                ...
                                <div>
                                    <a class="font-weight-bold text-dark" href="{{ route('lppa.index', ['jenis' => $jenisAktif->link, 'tahun' => $tahun->academicYearLink, 'anggaran' => $a->anggaran->link])}}">{{ $a->anggaran->name }}</a>
                                </div>
                            </div>
                            <div class="col-4 d-flex justify-content-end align-items-center">
                                <a href="{{ route('lppa.index', ['jenis' => $jenisAktif->link, 'tahun' => $tahun->academicYearLink, 'anggaran' => $a->anggaran->link])}}" class="btn btn-sm btn-outline-brand-green-dark">Pilih</a>
                            </div>
                        </div>
                    </div>
                    @endif
