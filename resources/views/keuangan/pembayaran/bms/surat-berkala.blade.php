...
<div class="page">
    ...
    @endphp
      ...
      <br><br>
    	...
    	<p class="p-b-8 lh-1-2"> <b>{{ $calon->identitas->student_name }} (Kelas {{$calon->level ? $calon->level->level_romawi : ''}} {{$calon->kelas ? $calon->kelas->jurusan ?$calon->kelas->jurusan->major_name.' ':'':''}}- {{$calon->kelas?$calon->kelas->namakelases->class_name:''}} )</b> </p>
      ...
      <br><br>
        ...
        <tr>
          ...
          <td class="p-b-0" style="width:14px">:</td>
          <td class="p-b-0">Rp {{ number_format($bms->bms_paid, 0, ',', '.') }}</td>
        </tr>
        @if($bms->bms_type_id == 2 || $bms->termin()->count() > 1)
        @foreach ($bms->termin as $index => $termin)
          <tr>
            <td class="p-b-0" style="width:205px">BMS berkala {{$index+1}}</td>
            <td class="p-b-0" style="width:14px">:</td>
            <td class="p-b-0">Rp {{ $termin->remain>0?number_format($termin->remain, 0, ',', '.'):'LUNAS' }}</td>
          </tr>
        @endforeach
        @endif
        <tr style="border-top: 1pt solid black;">
          ...
          <td class="p-b-0" style="width:14px">:</td>
          <td class="p-b-0" ><b>Rp {{ number_format($bms->bms_nominal - $bms->bms_paid, 0, ',', '.') }}</b></td>
        </tr>
        <tr>
          <td class="p-b-0" style="width:205px"><b>BMS yang harus dibayarkan</b></td>
          <td class="p-b-0" style="width:14px">:</td>
          <td class="p-b-0" colspan="2"><b>Rp 
            @php
                $ada = false;
                $ac_year = getAcademicYearActive();
            @endphp
            @foreach ($bms->termin as $termin)
                @if($termin->academic_year_id == $ac_year->id)
                    {{number_format($termin->remain, 0, ',', '.')}}
                    @php
                        $ada = true;
                    @endphp
                    @endif
            @endforeach
            @if (!$ada)
                {{number_format($bms->bms_remain, 0, ',', '.')}}
            @endif</b></td>
        </tr>
      </table>
      <br><br>
      <p class="p-b-8 lh-1-2">Sekiranya Ayah Bunda berkenan untuk memberikan konfirmasi pembayaran BMS tersebut paling lambat <b>pada hari {{$hari}}</b> kepada bagian Administrasi Keuangan {{$calon->unit->desc}} di nomor {{$calon->unit->phone_unit}} (via Telphone ) atau {{$calon->unit->whatsapp_unit}} (via WhatsApp ), dikarenakan belum tercatat pada Rekening Koran Auliya per tanggal {{'10 '.$month.' '.$year}}.</p>
      <br>
