<form action="{{ route($route.'.update') }}" id="edit-data-form" method="post" enctype="multipart/form-data" accept-charset="utf-8">
  ...
  <div class="row">
    <div class="col-12">
      <div class="form-group">
        <div class="row">
          <div class="col-12">
            <label for="editName" class="form-control-label">Kategori</label>
          </div>
          <div class="col-12">
            <select class="form-control @error('editStatus') is-invalid @enderror" name="editStatus" required="required">
              @foreach($status as $s)
              <option value="{{ $s->id }}" {{ $s->id == old('editStatus',$data->employee_status_id) ? 'selected' : null }}>{{ 'Anak '.$s->status }}</option>
              @endforeach
            </select>
          </div>
        </div>
  ...
  @else
  <div class="row mb-2">
    <div class="col-12 mb-1">Kategori</div>
    <div class="col-12">
      <h5>{{ $data->name }}</h5>
    </div>
  </div>
    ...
    <div class="col-12">
      <div class="form-group">
        <div class="row">
          ...
          <div class="col-12">
            <input type="text" id="editName" class="form-control @error('editName') is-invalid @enderror" name="editName" value="{{ old('editName',$data->name) }}"/>
          </div>
        </div>
      </div>
    </div>
