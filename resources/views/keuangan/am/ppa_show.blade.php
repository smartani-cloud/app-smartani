...
<tr id="p-{{ $p->id }}">
    ...
    && ($isAnggotaPa
     || (!$isAnggotaPa && ((in_array(Auth::user()->role->name, ['am']) && $ppaAktif->finance_acc_status_id == 1) || (in_array(Auth::user()->role->name, ['fam','faspv','am']) && $ppaAktif->type_id == 2))))
    && (
        (in_array(Auth::user()->role->name, ['fam','faspv','am']) && (($ppaAktif->type_id != 2 && $ppaAktif->finance_acc_status_id != 1) || $ppaAktif->type_id == 2))
        || (
            ($ppaAktif->type_id != 2 && ($ppaAktif->pa_acc_status_id != 1 || (in_array(Auth::user()->role->name, ['am']) && $ppaAktif->finance_acc_status_id == 1 && $ppaAktif->detail()->where('value','<=',0)->count() > 0 && !$ppaAktif->lppa))) || $ppaAktif->type_id == 2)
        )
    )
    <td>
		@if($ppaAktif->type_id == 2)
        ...
        @endif
        @elseif(($isAnggotaPa || (!$isAnggotaPa && in_array(Auth::user()->role->name, ['am']))) && $p->value <= 0 && !$ppaAktif->lppa)
        <a href="#" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#delete-confirm" onclick="deleteModal('Pengajuan', '{{ addslashes(htmlspecialchars(str_replace(array("\n", "\r"), '', $p->note))) }}', '{{ route('ppa.hapus.detail', ['jenis' => $jenisAktif->link, 'tahun' => !$isYear ? $tahun->academicYearLink : $tahun, 'anggaran' => $anggaranAktif->anggaran->link, 'nomor' => $ppaAktif->firstNumber, 'submitted' => $ppaAktif->is_draft == 1 ? null : '1', 'id' => $p->id]) }}')">
            <i class="fas fa-trash"></i>
        </a>
        @endif
