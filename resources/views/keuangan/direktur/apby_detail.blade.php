        ...
        <form action="{{ route('apby.perbarui',['jenis' => $jenisAktif->link, 'tahun' => !$isYear ? $tahun->academicYearLink : $tahun, 'anggaran' => $anggaranAktif->anggaran->link]) }}" id="apby-form" method="post" enctype="multipart/form-data" accept-charset="utf-8">
          {{ csrf_field() }}
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                ...
                @if($apbyAktif && $apbyAktif->finance_acc_status_id == 1 && $apbyAktif->detail()->whereHas('akun',function($q){$q->where(['is_fillable' => 1, 'is_static' => 0]);})->whereNull('director_acc_status_id')->count() > 0)
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($d->value_director && $d->value_director > 0 && $d->value_director != $d->value_president)
                                <i class="fa fa-lg fa-check-circle text-info mr-1" data-toggle="tooltip" data-html="true" data-original-title="Disetujui dengan perubahan oleh {{ Auth::user()->pegawai->is($d->accKetua) ? 'Anda' : $d->accKetua->name }}<br>{{ date('d M Y H.i.s', strtotime($d->president_acc_time)) }}<br>Awal: {{ $d->valueDirectorWithSeparator }}"></i>
                                @else
                            ...
                            <td>
                                @if($apbyAktif->finance_acc_status_id != 1 || $d->president_acc_status_id == 1 || ($d->president_acc_status_id != 1 && $d->akun->is_fillable < 1))
            ...
            <div class="card-footer">
                @if($apbyAktif->finance_acc_status_id == 1 && $apbyAktif->president_acc_status_id != 1 &&  $apbyAktif->detail()->whereHas('akun',function($q){$q->where(['is_fillable' => 1, 'is_static' => 0]);})->whereNull('president_acc_status_id')->count() > 0)
                <div class="row">
                    <div class="col-12">
