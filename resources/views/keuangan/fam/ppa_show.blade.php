...
<tr id="p-{{ $p->id }}">
    ...
    @if(($apbyAktif && $apbyAktif->is_active == 1) && ($isAnggotaPa || (!$isAnggotaPa && in_array(Auth::user()->role->name, ['fam']) && $ppaAktif->finance_acc_status_id == 1) || (!$isAnggotaPa && in_array(Auth::user()->role->name, ['fam','faspv']) && $ppaAktif->type_id == 2)) && ((in_array(Auth::user()->role->name, ['faspv']) && $ppaAktif->finance_acc_status_id != 1) || $ppaAktif->pa_acc_status_id != 1 || (in_array(Auth::user()->role->name, ['fam']) && $ppaAktif->finance_acc_status_id == 1 && $ppaAktif->detail()->where('value','<=',0)->count() > 0 && !$ppaAktif->lppa)))
    <td>
        ...
        @endif
        @endif
        @if(($isAnggotaPa && $p->finance_acc_status_id != 1) || (!$isAnggotaPa && in_array(Auth::user()->role->name, ['fam']) && $p->value <= 0 && !$ppaAktif->lppa))
