...
<tr id="p-{{ $p->id }}">
    ...
    @if(in_array(Auth::user()->role->name, ['fam','faspv','akunspv']) && ($apbyAktif && $apbyAktif->is_active == 1) && $ppaAktif->director_acc_status_id != 1)
    @php
    ...
    <td>
        ...
        @if(!$isKso && $p->employee_id == Auth::user()->pegawai->id && $p->edited_status_id == 1 && $p->editPegawai->is($p->accKeuangan))
        ...
        @if(($isKso && $p->value_fam && $p->value_fam > 0 && $p->value_fam != $p->value_director) || (!$isKso && $p->employee_id == Auth::user()->pegawai->id && $p->edited_status_id == 1 && $p->editPegawai->is($p->accDirektur)))
    ...
    @if(($apbyAktif && $apbyAktif->is_active == 1) && (($isAnggotaPa && $p->finance_acc_status_id != 1) || (!$isAnggotaPa && in_array(Auth::user()->role->name, ['fam']) && $ppaAktif->finance_acc_status_id == 1) || (!$isAnggotaPa && in_array(Auth::user()->role->name, ['fam','faspv']) && $ppaAktif->type_id == 2)) && (($ppaAktif->pa_acc_status_id != 1 || (in_array(Auth::user()->role->name, ['fam']) && $ppaAktif->finance_acc_status_id == 1 && $ppaAktif->detail()->where('value','<=',0)->count() > 0))))
    <td>
        ...
        <a href="#" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#delete-confirm" onclick="deleteModal('Pengajuan', '{{ addslashes(htmlspecialchars(str_replace(array("\n", "\r"), '', $p->note))) }}', '{{ route('ppa.hapus.detail', ['jenis' => $jenisAktif->link, 'tahun' => !$isYear ? $tahun->academicYearLink : $tahun, 'anggaran' => $anggaranAktif->anggaran->link, 'nomor' => $ppaAktif->firstNumber, 'submitted' => $ppaAktif->is_draft == 1 ? null : '1', 'id' => $p->id]) }}')">
...
@if($isPa && $apbyAktif->is_final != 1 && ($ppaAktif->pa_acc_status_id != 1 || $ppaAktif->detail()->where(function($q){$q->where('pa_acc_status_id','!=',1)->orWhereNull('pa_acc_status_id');})->count() > 0))
  <div class="row">
      <div class="col-12">
