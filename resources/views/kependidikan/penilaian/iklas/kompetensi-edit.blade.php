...
<div class="alert alert-warning" role="alert">
  <strong>Perhatian!</strong> Perubahan kompetensi mungkin akan berpengaruh pada rapor terdahulu
</div>
@endif
<form action="{{ route($route.'.update',['unit' => $unit->name]) }}" id="update-form" method="post" enctype="multipart/form-data" accept-charset="utf-8">
  ...
  @if(isset($unitEditable) && $unitEditable && isset($unit) && auth()->user()->role->name != 'kepsek')
  <div class="row">
    <div class="col-12">
  ...
  <div class="row">
    <div class="col-12">
      <div class="form-group">
        <div class="row">
          <div class="col-12">
            <label for="editName" class="form-control-label">Kompetensi</label>
          </div>
          <div class="col-12">
            <input type="text" id="editName" class="form-control" name="editName" maxlength="50" required="required" value="{{ $data->name ? $data->name : '' }}">
          </div>
        </div>
      </div>
    </div>
